...
# Apply summarise_all
wide_gapminder %>%
  dplyr::select(-c(1:4)) %>% # STEP1 
  summarise_all(mean, na.rm = TRUE) # STEP2
```

Could you summarize the average gdp per capita per each country?
...
# What the shared variables between flights and planes? 
# If you want to join these two tables, how can you do it and why?
...
left_join(flights, planes, by = c("tailnum")) %>% is.na() %>% mean()
inner_join(flights, planes, by = c("tailnum")) %>% is.na() %>% mean()
```
