...
The function for recovering object classes is ```class()```. L suffix to qualify any number to make it an explicit integer. See more from the [R language definition](https://cran.r-project.org/doc/manuals/R-lang.html).
...
All atomic vector elements must be the same type, so when you combine different types, they will be __coerced__ to the **most flexible type.** Types from least to most flexible are: logical > integer > double > character. 
...
Names don't have to be unique. However, character subsetting, described in the next lesson, is the most important reason to use names, and it is most useful when the names are unique. (For Python users, a vector behaves like a Python dictionary key when names are unique.)
