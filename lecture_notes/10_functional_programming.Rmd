...
```{r}
if (x < 0) { # condition one 
  print("x is negative") # computation = print()
} else { # condition two 
  print("x is positive or zero") # computation = print()
}
```
...
while (i <= length(fruits)) { # condition
  print(fruits[i])
  i <- i + 1
}
...
tic()
for (i in 1:length(fruits_large)) { # range
  print(fruits_large[i])
}
toc()

tic()
map(.x = fruits_large, .f = print)
toc()
```
...
```{r}
df %>%
  map_df(~sum(is.na(.x))) # one more trick 
```
